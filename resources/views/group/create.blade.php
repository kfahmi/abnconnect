...
@section('content')
<!-- .row -->
{!! Form::open(array('route' => 'group.store', 'method' => 'POST', 'role' => 'form', 'class' => 'needs-validation')) !!}
    ...
    <div class="white-box" id="custom_permission">
        <div class="form-group {{$errors->has('permission_selected') ? 'has-error' : ''}}">
            ...
            <span class="col-lg-4 checkbox-inline checkbox-primary checkbox-circle">
                @if(old('permission_selected') !== null)
                <input class="checkbox-{{$p->id}}" name="permission_selected[]" type="checkbox" value="{{$p->id}}" {{in_array($p->id, old('permission_selected')) ? 'checked' : ''}}>
                @else
